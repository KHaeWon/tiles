...
public class BoardServiceImpl implements BoardService {
	...
	public List<BoardDTO> findByCategory(String category) {
		...
		if(category!=null) {//category!=null이어야 작업할 거임!
	...
	// 두 개의 작업을 처리
	// tbBoard에 글에 대한 내용을 저장, board_file 테이블에 첨부파일의 내용들을 저장
	// dao에 정의된 메소드 2개를 호출
	// 서비스 메소드에서 호출되는 2개의 메소드가 모두 정상 처리 되어야 db에 반영될 수 있도록 처리해야 한다.
	// 만약 두 작업 하나의 작업만 처리가 되고 오류가 발생되면 모두 작업이 취소되도록 처리해야 한다.
	// 논리적인 작업(작업 1개)
	// --------> 트랜잭션 처리
	@Override
